...
const AuthState = ({ fontSize = "1em" }) => {
  ...
  return (
    <div style={{ fontSize }}>
      ...
      {user && !isProducer && ( // Check for both user and !isProducer
        <>
          <button className="startselling" onClick={() => setIsModalOpen(true)}>
            start selling
          </button>
          <ReactModal
            isOpen={isModalOpen}
          ...
          >
            ...
            <p>Are you sure you want to start selling?</p>
            <div className="modal-buttons">
              <button onClick={handleStartSelling} className="confirm-button">
                Yes
              </button>
          ...
          <ReactModal
            ...
            onRequestClose={() => setSuccessModalOpen(false)} // Allow closing manually
            className="modal"
            overlayClassName="overlay"
            ariaHideApp={false} // Disable accessibility warnings
          >
            <h2>Success</h2>
            <p>You are now a producer!</p>
          </ReactModal>
        </>
      )}
    </div>
